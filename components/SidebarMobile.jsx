...
const SidebarMobile = ({ setBurgerIsOpen }) => {
  ...
  return (
    <div className="font-rubik text-sm md:text-base absolute top-[69px] md:top-[80px] left-[0px] w-full min-h-[calc(100vh-69px)] md:h-[calc(100vh-80px)] z-40 unselectable">
      <div className="w-full min-h-[calc(100vh-60px)] bg-neutral-100 dark:bg-neutral-900 flex flex-col items-start">
            ...
            >
              <Link className='w-full flex items-center gap-3' href={icon.link}>
                <icon.element
                  ...
                  height={28}
                />
                <p className={pathname === icon.link ? 'text-white text-lg' : 'text-lg'}>{icon.title}</p>
              </Link>
            </div>
          </div>
        )}
